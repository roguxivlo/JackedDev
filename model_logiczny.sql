...
CREATE TABLE exercise (
    ...
    CHECK (difficulty_level='Beginner' OR 
        difficulty_level='Intermediate' OR
        difficulty_level='Advanced')
);

CREATE TABLE exercise_description (
    id NUMBER(4) FOREIGN KEY REFERENCES exercise(id),
...
CREATE TABLE required_equipment (
    exercise_id NUMBER(4) FOREIGN KEY REFERENCES exercise(id),
    equipment_id NUMBER(4) FOREIGN KEY REFERENCES equipment(id),
...
CREATE TABLE used_muscle (
    exercise_id NUMBER(4) FOREIGN KEY REFERENCES exercise(id),
    muscle_id NUMBER(4) FOREIGN KEY REFERENCES muscle(id),
